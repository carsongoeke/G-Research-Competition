...
set.seed(123)
deep_boost <- xgboost(data = x_train,
                         label = y_train,
...
set.seed(123)
medium_boost <- xgboost(data = x_train,
                      label = y_train,
...
# Predictions -------------------------------------------------------------------- 
predictions <- cbind(predict(deep_boost, x_test), predict(medium_boost, x_test))
avg_preds <- apply(predictions, 1, mean)
dnn_predictions <- cbind(test$Index, avg_preds) %>% as.data.frame()
...
# Export Predictions
write.csv(dnn_predictions, '~/Desktop/gresearch/boost_combo.csv', na = '', row.names = FALSE)
