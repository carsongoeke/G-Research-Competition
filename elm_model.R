...
n_elms <- 50
bagged_elm_predictions <- matrix(0, nrow = nrow(x_test), ncol = n_elms)
bagged_elm_train_preds <- matrix(0, nrow = nrow(x_train), ncol = n_elms)

for (i in 1:n_elms) {
  
  # bagging indices and random subspace
  boot_ind <- sample(1:nrow(x_train), nrow(x_train), prob = weights_train)
  rand_col <- sample(1:ncol(x_train), 0.3*ncol(x_train))

  x_train_boot <- x_train[boot_ind, rand_col]
  ...
  # Record Predictions
  bagged_elm_predictions[,i] <- predict(elm_model, x_test)
  bagged_elm_train_preds[,i] <- predict(elm_model, x_train)
  
}

# Evalulate model
wse <- (abs(y_train - as.numeric(apply(bagged_elm_train_preds, 1, mean))) * weights_train) %>% sum()
paste('Weighted Sum of Errors', wse)
...
# Export Predictions
write.csv(elm_preds, '~/Desktop/gresearch/elm_50bag_64n_0.3randsub_minmax_.csv', na = '', row.names = FALSE)
