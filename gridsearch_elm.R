...
for (r in 1:n_elms) {
  ...
  rand_col <- sample(1:ncol(x_train), rand_sub*ncol(x_train))
  ...
  bagged_elm_train_preds[,r] <- predict(elm_model, x_train[,rand_col])
  
}

# Evalulate model
wse <- (abs(y_train - as.numeric(apply(bagged_elm_train_preds, 1, mean))) * weights_train)
wse %<>% as.numeric()
wse %<>% sum()
cat('Weighted Sum of Errors:', wse, '\n',
    'n elm:', n_elms, '\n',
    'n hidden:', n_hidden, '\n',
    'rand sub:', rand_sub, '\n\n')
